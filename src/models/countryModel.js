...
/**
 * Convert database row to proper types
 */
function formatCountryData(country) {
    ...
    return {
        ...country,
        exchange_rate: country.exchange_rate !== null ? parseFloat(country.exchange_rate) : null,
        estimated_gdp: country.estimated_gdp !== null ? parseFloat(country.estimated_gdp) : null
    };
}
...
async function getAllCountries(filters = {}) {
    ...
    return rows.map(formatCountryData);
}
...
async function getCountryByName(name) {
    ...
    return formatCountryData(rows[0]);
}
...
async function getTopCountriesByGDP(limit = 5) {
    ...
    return rows.map(row => ({
        name: row.name,
        estimated_gdp: row.estimated_gdp !== null ? parseFloat(row.estimated_gdp) : null
    }));
}
